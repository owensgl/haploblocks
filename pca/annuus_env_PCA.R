...
folder <- "/media/owens/Copper/wild_gwas/annuus/"
gds.file <- "Annuus.tranche90.snp.env.90.bi.remappedHa412HO.gds"
vcf.file <- "Annuus.tranche90.snp.env.90.bi.remappedHa412HO.vcf.gz"
...
write(pca$eigenval, "PCA/Annuus.tranche90.snp.env.90.bi.remappedHa412HO.ldr0p2.eigenvalues.txt",
      ncol=length(pca$eigenval))


write.table(pca$eigenvect, "PCA/Annuus.tranche90.snp.env.90.bi.remappedHa412HO.ldr0p2.eigenvectors.txt",
...
#Plotting PCAs
pdf("Annuus.tranche90.snp.env.90.bi.ldr0p2.remappedHa412HO.pca.pdf",height=6,width=14)
for (i in seq(1,20,2)){
  j <- i +1
...
write_tsv(tab, "Annuus.tranche90.snp.env.90.bi.remappedHa412HO.ldr0p2.pca.txt")
...
pdf("Annuus.tranche90.snp.env.90.bi.remappedHa412HO.ldr0p2.pca.loadings.pdf")
for (i in 1:20){
...
tab %>%
  ...
  ggplot(.,aes(x=EV1,y=EV2)) + geom_point(aes(color=as.factor(texanus)))
tab %>%
  inner_join(sample_info) %>%
  inner_join(texanus_populations) %>%
...
write_delim(selected_snps, "Annuus.tranche90.snp.env.90.bi.ldr0p2.txt")
...
pdf("Annuus.tranche90.snp.env.90.bi.ldr0p2.kinship.pdf")
...
write_delim(ibd.dat, "Annuus.tranche90.snp.env.90.bi.ldr0p2.kinship.txt")
...
folder <- "/media/owens/Copper/wild_gwas/annuus/"
gds.file <- "Annuus.tranche90.snp.env.90.bi.remappedHa412HO.v3noinv.ldfilter.gds"
vcf.file <- "Annuus.tranche90.snp.env.90.bi.remappedHa412HO.v3noinv.ldfilter.vcf.gz"
...
#Write eigenvectors and eigenvalues
write(pca$eigenval, "PCA/Annuus.tranche90.snp.env.90.bi.remappedHa412HO.v3noinv.ldfilter.ldr0p2.eigenvalues.txt",
      ncol=length(pca$eigenval))


write.table(pca$eigenvect, "PCA/Annuus.tranche90.snp.env.90.bi.remappedHa412HO.v3noinv.ldfilter.ldr0p2.eigenvectors.txt",
            col.names = F,row.names = F
)
...
#Plotting PCAs
pdf("PCA/Annuus.tranche90.snp.env.90.bi.remappedHa412HO.v3noinv.ldfilter.ldr0p2.pca.pdf",height=4,width=14)
for (i in seq(1,20,2)){
  j <- i +1
  tab <- data.frame(name = pca$sample.id,
                    EV1 = pca$eigenvect[,i],  
...
write_tsv(tab, "PCA/Annuus.tranche90.snp.env.90.bi.remappedHa412HO.v3noinv.ldfilter.ldr0p2.pca.txt")
...
tab %>%
  ...
  inner_join(texanus_populations) %>% 
  ungroup() %>%
  mutate(target = case_when(pop == "ANN_52" ~ "Yes",
                            pop != "ANN_52" ~ "No")) %>% 
  ggplot(.,aes(x=EV1,y=EV2)) + geom_point(aes(color=as.factor(texanus)))
tab %>%
  inner_join(sample_info) %>%
